...
class MainPage(webapp2.RequestHandler):
    ...
    def get(self):
        ...
        if user:
        	...
        	user = user.nickname()
        	params = {'owner': user}
        	url = 'http://wenjingxie1121.appspot.com/manageAPI'
...
class DeleteOwnStream(webapp2.RequestHandler):
	def post(self):
		...
		url = 'http://wenjingxie1121.appspot.com/deleteAPI'
		result = jsonfyFetch(params, url)
		self.redirect('/')
...
class CreateSubmit(webapp2.RequestHandler):
	...
	def post(self):
		...
		url = 'http://wenjingxie1121.appspot.com/createSubmitAPI'
		result = jsonfyFetch(params,url)
...
class ViewAll(webapp2.RequestHandler):
	...
	def get(self):
		...
		if user:
	 		params = {'task': 'viewAll'}
	 		url = 'http://wenjingxie1121.appspot.com/viewAllAPI'
	 		result = jsonfyFetch(params, url)
			self.response.write("""<!doctype html><html><body>
        		<h1> Connex.us </h1>
        		<p>
...
class ViewSingle(webapp2.RequestHandler):
	def get(self, streamKey, imageKey, viewCount):
		...
		imageKey = blobstore.BlobKey(imageKeyStr)
 		...
 		if len(images) == 0:
 			...
 			if (index + 1) <= 3:
 				while index >= 0:
 					...
 					url = 'http://wenjingxie1121.appspot.com/getUrl'
 		...
 		# Add new pics to this stream
 		url = '/upload%s'%streamKeyUrl
 		upload_url = blobstore.create_upload_url(url)
 		self.response.out.write('<p><h3> Add an Image:</h3> <form action="%s" method="POST" enctype="multipart/form-data">' % (upload_url))
 		self.response.out.write("""File Name: <input type="file" name="file"><br> <input type="submit"
        name="submit" value="Upload file"></form></p>""")

        # subscribe
 		self.response.write("""<p><a href = "/subscribe/%s/%s"> <form action="">
                          <input type="button" value="Subscribe">
                          </form></a></p>"""%(streamKeyUrl, imageKeyStr))
		self.response.write("</body></html>")
		if viewCount == '1':
...
class Subscribe(webapp2.RequestHandler):
	...
	def get(self, streamKey, imageKey):
		...
		url = 'http://wenjingxie1121.appspot.com/subscribeAPI'
		result = jsonfyFetch(params, url)
...
class Search(webapp2.RequestHandler):
	def get(self):
		...
		self.response.write("""<form name="input" action="/searchSubmit" method="get">
                Keyword to search: <input type="text" name="keyword"><br>
...
class SearchSubmit(webapp2.RequestHandler):
	...
	def get(self):
		...
		url = 'http://wenjingxie1121.appspot.com/searchAPI'
		result = jsonfyFetch(params, url)
...
class Trending(webapp2.RequestHandler):
	def get(self):
		global frequency
		params = {}
		url = 'http://wenjingxie1121.appspot.com/trendingAPI'
		result = jsonfyFetch(params, url)
